...
**CDOC2 Shares Server**
: CDOC2 Shares Server mediates Key Shares Capsules between Sender and Recipient. Sender's Client can upload a Server Capsule to one or multiple CCS servers. Recipient's Client can download Server Capsule from CCS server after authentication.
...
**Shares Capsule**
: Encryption/decryption key material which is split into shares in order to distribute it ton multiple CSS servers.
...
**Use Case Context**
: CDOC2 Client Application adds Sender's chosen files into the CDOC2 container and encrypts the container with CEK. CEK is encrypted with KEK. KEK is generated by Sender client and then divided into shares. The secret shared decryption key is distributed to multiple CDOC2 Shares Servers.
...
7. Client forwards a share per Recipient together with their natural person identifier to each of the CDOC2 Shares Servers. Client provides a shares capsule expiration time from internal application configuration for each capsule share.  Client receives a transaction code for each shares capsule share.
9. Client creates a container into file system in the chosen target path and adds a header.
10. Client verifies that the header does not exceed the size limit defined by the specification.
11. Client verifies technical file correctness and file name safety rules according to the specification. Client creates an archive, compresses it, encrypts the compressed archive with CEK and adds it to the container as payload.
12. Client saves the CDOC2 container and displays Sender a notification.
...
1. Client first syncs default capsule expiration time from an organization-specific external service.
2. External configuration service provides a default capsule expiration time.
3. Client sends a shares capsule share to CSS API service using the expiration time from external configuration. Client receives a transaction code for each shares capsule share.
4. Use case continues from step 9.
...
4. Client sends the correct CDOC2 Shares Server the transaction code for each shares capsule share and receives a nonce from each server.
...
12. Client receives the shares capsule share from each CSS. 
...
3. Each CSS generates a universally unique transaction code (UUID).
4. Each CSS saves the shares capsule share, validates the expiration time provided by Client based on its system configuration settings and sets the expiration time of the capsules.
5. Each CSS returns Client a transaction code for each capsule.
...
1. Client first syncs default capsule expiration time from an organization-specific external service.
2. External provides a default capsule expiration time.
3. Client sends a shares capsule share using the appropriate API service to all CSS-s using the expiration time from external configuration or when a Recipient's certificate expiration time is earlier, it uses the certificate expiration time for that Recipient's capsule share.
4. Use case continues from step 2.
...
1. CSS calculates the expiration time based on system configuration.
2. Use case continues from step 5.

4b. The expiration time provided by Client is longer than allowed in CSS system configuration:
...
**Context of Use**
: CDOC2 Client Application requests Shares Capsule shares from all CDOC2 Shares Servers. The Shares Capsule share is identified by the transaction code provided by CDOC2 Client Application, the CSS server ID and the public key in Recipient certificate.
...
6. CSS returns the Shares Capsule share. 
...
5a. Validation by a CSS finds an error:
