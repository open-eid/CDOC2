...
"CDOC2 system" is a system containing two distinct operational contexts, described separately below. Both contexts share the CDOC2 reference library and CDOC2 CLI client as common components.
...
## Hardware Token/Capsule Server Context
...
3. CDOC2 Authentication Server (cdoc2-auth-server): used in SID/MID autentication flows only. Composes and issues Session Ttokens that are included as headers in subsequent requests to other CDOC2 infrastructure components. Has a local database component.
...
6. CDOC2 CLI client: a command-line Java application and which implements all CDOC2 end-user use cases, but without graphical user interface.
