...
Design of SC06 might be a bit confusing at first, as generating two different salt values
(`PasswordSalt_i` and `KeyMaterialSalt_i`) doesn't seem to offer additional security. Rationale
behind this design choice is the following:

1. protocol designs of SC05 and SC06 should be similar, because they are dealing with similar situations. If SC05 uses `KeyMaterialSalt_i`,  SC06 should also use that.
...
Can we do without salt values? Can we re-use a single salt value with PBKDF2 and HKDF? Difficult to decide. However, as generating two separate salt values is not expensive and certainly shouldn't do  harm, it was decided to err on the side of caution, rather than strictly follow individual RFC recommendations and therefore, to use two independent salt values.
